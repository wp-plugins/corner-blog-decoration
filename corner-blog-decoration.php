...
if ( ( get_option( 'cbd_str' ) && get_option( 'cbd_left_right' ) && get_option( 'cbd_top_bottom' )
 && get_option( 'cbd_left_right_px' ) && get_option( 'cbd_top_bottom_px' ) ) == false ) {
cbd_first();
}

echo('
<div class="wrap">
<h2>Corner Blog Decoration Options</h2>
');
...
if ( $cbd ) echo('<div class="updated"><p><strong>Your setting has been save.</strong></p></div>');
else echo('<div class="error"><p><strong>Your setting not save.</strong></p></div>');
}

echo('
<form action="" method="post">
<table class="form-table">
<tr><td>HTML to insert<br/><span class="cbd_admin_hint">(Insert "&lt;a&gt;" tag, "&lt;img&gt;" tag, or javascript)<br/></span></td><td><textarea name="cbd_str" rows="5" cols="50">' . get_option( 'cbd_str' ) . '</textarea></td></tr>
<tr><td>Horizontal Position<br/><span class="cbd_admin_hint">("Left" or "Right")</span></td><td><select name="cbd_left_right">
');
switch( get_option( 'cbd_left_right' ) ) {
case 'left' :
echo('<option value="left" selected="true">Left</option>
...
echo('</select>&nbsp;<input type="text" name="cbd_left_right_px" value="' . get_option( 'cbd_left_right_px' ) . '" size="4" />&nbsp;px</td></tr>
<tr><td>Vertical Position<br/><span class="cbd_admin_hint">("Top" or "Bottom")</span></td><td><select name="cbd_top_bottom">
');
switch( get_option( 'cbd_top_bottom' ) ) {
case 'top' :
echo('<option value="top" selected="true">Top</option>
...
echo('</select>&nbsp;<input type="text" name="cbd_top_bottom_px" value="' . get_option( 'cbd_top_bottom_px' ) . '" size="4" />&nbsp;px</td></tr>
<tr><td><input class="button-primary" type="submit" name="submit" value="Save Setting" /></td><td>&nbsp;</td></tr>
</table>
</form>
</div>
');
